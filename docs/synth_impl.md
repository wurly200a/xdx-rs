...
| パラメータ | 型 | 効果 | 優先理由 |
|---|---|---|---|
| `kbd_lev_scl` (Operator) | 0-99 | ノート番号でオペレーター出力レベルをスケール | 固定ノート録音でもOP間バランスが変化。差の有力候補 |
| `kbd_rate_scl` (Operator) | 0-3 | ノート番号でEGレート(AR/D1R/D2R/RR)を加速 | 高音域ほどEGが速くなる |
...
```
Step 1  kbd_lev_scl / kbd_rate_scl  ← EG比較精度の最優先課題
