...
impl<T> MidiInputConnection<T> {
    ...
    fn close_internal(&mut self) {
        // FIX: Acquire the lock only to copy the raw handle, then release it
        // BEFORE calling midiInReset.  midiInReset fires MIM_LONGDATA callbacks
        // (one per pending sysex buffer) which also try to acquire this lock to
        // call midiInAddBuffer.  If we hold the lock across midiInReset we get a
        // cross-thread deadlock: close thread holds lock → WinMM callback thread
        // waiting for lock → midiInReset never returns.
        // Releasing the lock first lets the callbacks complete; midiInStop then
        // ensures no new callbacks fire before we re-acquire for the rest of cleanup.
        //
        // Original note about the lock:
        // https://groups.google.com/forum/#!topic/mididev/6OUjHutMpEo
        let in_handle = {
            let guard = self.handler_data.in_handle.as_ref().unwrap().0.lock();
            *guard
            // guard dropped here — lock released before midiInReset
        };

        unsafe {
            midiInReset(in_handle);  // fires callbacks; they can now acquire lock
            midiInStop(in_handle);   // no new callbacks after this
        }

        // Re-acquire for the header cleanup and close.
