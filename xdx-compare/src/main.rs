...
fn main() {
    ...
    let midi_note: u8 = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(69);
    ...
    std::fs::create_dir_all(&out_dir).unwrap_or_else(|e| panic!("Cannot create output dir: {e}"));
    ...
    if midi_out.is_some() && audio_in.is_some() {
        ...
        let gain = if synth.sustain_rms > 0.0 {
            dx100.sustain_rms / synth.sustain_rms
        } else {
            1.0
        };
        ...
        println!(
            "  crest-factor dx100-synth : {:+.4}  (>0 = dx100 more harmonic than synth)",
            cf_diff
        );
        if dx100.peak < 0.01 {
        ...
        if audio_in.is_none() {
            println!("(skip DX100 recording: --audio-in not specified)");
        }
...
fn print_stats(label: &str, s: &AudioStats) {
    println!(
        "  {label}  rms={:.4}  peak={:.4}  sustain_rms={:.4}  crest={:.4}",
...
fn render_synth(
    ...
    let sustain_rms = if sus_n > 0 {
        (sus_sq / sus_n as f64).sqrt() as f32
    } else {
        0.0
    };
