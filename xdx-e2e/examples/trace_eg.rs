    ...
    fn tick(&mut self) {
        match self.stage {
            Stage::Attack => {
                ...
                if self.level >= 1.0 { self.stage = Stage::Decay1; }
            }
            Stage::Decay1 => {
                self.level = (self.level * self.d1r_mul).max(self.d1l);
                if self.level <= self.d1l + 1e-6 { self.stage = Stage::Decay2; }
            }
            Stage::Decay2 => {
                self.level = (self.level * self.d2r_mul).max(0.0);
                if self.level < 1e-5 && self.d2r_mul < 1.0 { self.stage = Stage::Off; }
            }
            _ => {}
        }
    }
...
fn main() {
    ...
    println!("{:>8}  {:>6}  {:>6}  {:>6}  {:>6}  {:>6}  {:>8}  note",
        "t_ms", "st1", "lv1", "st2", "lv2", "st3", "st4");
    println!("{}", "-".repeat(72));

    let base_hz = 440.0_f32 * 2.0_f32.powf((MIDI_NOTE as f32 + (voice.transpose as f32 - 24.0) - 69.0) / 12.0);
    ...
    for s in 0..hold_samples {
        ...
        if s % print_interval == 0 || stage_changed {
            ...
            let marks: Vec<String> = (0..4).map(|i| {
                if envs[i].stage != prev_stages[i] {
                    format!(" OP{} {:?}→{:?}", i+1, prev_stages[i], envs[i].stage)
                } else { String::new() }
            }).collect();
            println!("{:>8.1}  {:>6.4}  {:>6.4}  {:>6.4}  {:>6.4}{}",
                t_ms,
                envs[0].level, envs[1].level, envs[2].level, envs[3].level,
                marks.concat(),
            );
        }
        for i in 0..4 { prev_stages[i] = envs[i].stage; }
    }
    println!();
    ...
    for (i, op) in voice.ops.iter().enumerate() {
        ...
        let peak_samples = if ar_inc > 0.0 { (1.0 / ar_inc).ceil() as u64 } else { u64::MAX };
        ...
        println!("  OP{}: peak at {:.2}ms  amp={:.5}  freq={:.2}Hz  (ratio={} det={}ct)",
            i+1, peak_ms, amp, freq, ratio, det_cents as i32);
    }
}
