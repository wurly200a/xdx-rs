...
mod real {
    ...
    impl MidiManager {
        ...
        pub fn list_in_ports() -> Vec<String> {
            let Ok(mi) = MidiInput::new("xdx-list") else {
                return vec![];
            };
            mi.ports()
                .iter()
        ...
        pub fn list_out_ports() -> Vec<String> {
            let Ok(mo) = MidiOutput::new("xdx-list") else {
                return vec![];
            };
            mo.ports()
                .iter()
        ...
        pub fn open_in(&mut self, port_name: &str) -> Result<(), MidiError> {
            self.close_in();
            let mut mi = MidiInput::new("xdx-in").map_err(|e| MidiError(e.to_string()))?;
        ...
        pub fn open_out(&mut self, port_name: &str) -> Result<(), MidiError> {
            self.close_out();
            let mo = MidiOutput::new("xdx-out").map_err(|e| MidiError(e.to_string()))?;
        ...
        pub fn out_connected(&self) -> bool {
            self.out_tx.is_some()
        }
    }
...
mod stub {
    ...
    impl MidiManager {
        ...
        pub fn send(&mut self, _data: &[u8]) -> Result<(), MidiError> {
            Ok(())
        }
